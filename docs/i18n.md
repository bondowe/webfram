...
app.Configure(&app.Config{
    Assets: &app.Assets{
        ...
        I18nMessages: &app.I18nMessages{
            ...
            SupportedLanguages: []string{"en", "fr", "es", "de"}, // Optional: auto-detected if omitted
        },
    },
})

// Option 2: Load from working directory (useful for development)
app.Configure(nil)  // i18n messages loaded from ./assets/locales with auto-detection
```

**Note:** All i18n configuration is optional. When `Assets` or `I18nMessages` are omitted, the framework:
- Loads message files from `./assets/locales` relative to the working directory
- Auto-detects supported languages from available `messages.*.json` files
...
WebFram auto-detects languages from message files when `SupportedLanguages` is not specified:
...
```go
// All these configurations enable auto-detection:

// 1. Minimal (working directory)
app.Configure(nil)
...
app.Configure(&app.Config{
    Assets: &app.Assets{
        ...
        I18nMessages: &app.I18nMessages{
            Dir: "assets/locales",
            // SupportedLanguages auto-detected from message files
        },
    },
})

// 3. Working directory with custom path
app.Configure(&app.Config{
    Assets: &app.Assets{
        I18nMessages: &app.I18nMessages{
            Dir: "locales",  // Custom path, FS defaults to working directory
        },
    },
})
```
